---
title: "COVIDCNS FBC Cleaning Script"
author: "Zain Ahmad"
date: "02/07/2022"
...
# Read in the data: COVIDCNS Full Blood Count
...
covidcns_dat <- read_rds(
  file = paste0(ilovecovidcns, "/data_raw/latest_freeze/fbc/fbc_covid_cns.rds")
  )
...
covidcns_dat_id <- covidcns_dat %>% #new dataset with ID
  ...
  dplyr::rename(
    ...
    "fbc.red_cell_count" = "fbc.fbc_results_levels_report",
    "fbc.hemoglobin" = "fbc.fbc_results_levels_report.1",
    "fbc.hematocrit" = "fbc.fbc_results_levels_report.2",
    "fbc.mean_cell_volume" = "fbc.fbc_results_levels_report.3",
    "fbc.mch" = "fbc.fbc_results_levels_report.4",
    "fbc.mchc" = "fbc.fbc_results_levels_report.5",
    "fbc.rdw" = "fbc.fbc_results_levels_report.6",
    "fbc.platelets" = "fbc.fbc_results_levels_report.7",
    "fbc.mean_platelet_volume" = "fbc.fbc_results_levels_report.8",
    "fbc.neutrophils" = "fbc.fbc_results_levels_report.9",
    "fbc.lymphocytes" = "fbc.fbc_results_levels_report.10",
    "fbc.monocytes" = "fbc.fbc_results_levels_report.11",
    "fbc.eosinophils" = "fbc.fbc_results_levels_report.12",
    "fbc.basophils" = "fbc.fbc_results_levels_report.13",
    "fbc.nucleated_rbcs" = "fbc.fbc_results_levels_report.14",
    "fbc.hypochromic_rbcs" = "fbc.fbc_results_levels_report.15"
  )
...
variables_cont <- c(
  ...
  "fbc.hemoglobin",
  "fbc.hematocrit",
  "fbc.mean_cell_volume",
  "fbc.mch",
  "fbc.mchc",
  "fbc.rdw",
  "fbc.platelets",
  "fbc.mean_platelet_volume",
  "fbc.neutrophils",
  "fbc.lymphocytes",
  "fbc.monocytes",
  "fbc.eosinophils",
...
dat <- dat %>%
  mutate(across(fbc.red_cell_count,
                ~case_when(str_detect(fbc.red_cell_count, "3.49\\ x\\ 10\\^12\\/L") ~ "3.49",
                           str_detect(fbc.red_cell_count, "4.18 10E12/L") ~ "4.18",
                           str_detect(fbc.red_cell_count, "4.18 10E12/L") ~ "4.18",
                           TRUE ~ .)
                )
         )
...
dat <- dat %>%
  mutate(across(fbc.platelets,
                ~case_when(str_detect(fbc.platelets, "192 10E9/L") ~ "192",
                           TRUE ~ .)
                )
         )
...
fbc_rr <- read_csv(file = "scripts/ref_ranges/fbc_rr.txt",
                   col_types = list(
...
limits_mat <- as.matrix(fbc_rr[,4:5])
...
for (i in 1:ncol(out_tib)){
  ...
  out_tib[i] <- set_label(out_tib[i], fbc_rr[i, 2])
}
...
dat %>%
  saveRDS(
    file = paste0(ilovecovidcns, "/data/latest_freeze/fbc/fbc_covidcns_clean.rds")
    )
```