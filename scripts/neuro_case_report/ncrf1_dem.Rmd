...
covidcns_dat <- read_rds(
  file = paste0(ilovecovidcns, "/data_raw/latest_freeze/neuro_case_report/ncrf1_dem_covid_cns.rds")
  )
...
lower_limit <- as.POSIXct("1905-10-27")
```
...
```{r Recode country of birth variable}
dat <- dat %>% 
  mutate(across(ncrf1_dem.country_of_birth.txt,
                ~case_when(
                  ...
                  str_detect(ncrf1_dem.country_of_birth.txt, "rinid")
                  ~ "Trindad & Tobago",
...
Re-check country of birth
Repeat these chunks as necessary
```{r Recheck country of birth}
dat %>%
  select(ncrf1_dem.country_of_birth.txt) %>%
  tbl_summary(missing_text = "Missing")
```
...
dat <- dat %>% 
  mutate(ncrf1_dem.simplified_ethnicity =
                case_when(
                  ...
                  str_detect(ncrf1_dem.ethnicity.txt, "nk") ~ "Not Known",
                  str_detect(ncrf1_dem.ethnicity.txt, "NK") ~ "Not Known",
                  str_detect(ncrf1_dem.ethnicity.txt, "ot") ~ "Not Known")
         )
...
Re-check ethnicity
Repeat these chunks as necessary
```{r Recheck coding ethnicity}
dat %>%
  select(ncrf1_dem.simplified_ethnicity) %>%
  tbl_summary(missing_text = "Missing")
```
