---
title: "COVIDCNS NCRF2 Care Cleaning Script"
author: "Zain Ahmad"
date: "14/06/2022"
...
# Read in the data: COVIDCNS Neuro Case Report Form Module 2 Supportive Care 
...
covidcns_dat <- read_rds(
  file = paste0(ilovecovidcns, "/data_raw/latest_freeze/neuro_case_report/ncrf2_care_covid_cns.rds")
  )
...
covidcns_dat_id <- covidcns_dat %>% #new dataset with ID
  ...
  dplyr::rename(
    ...
    "ncrf2_care.modified_rankin_score_children.txt" = "QID167_TEXT"
  )
...
variables_cont <- c(
  "ncrf2_care.overall_duration_of_invasive_ventilation.txt",
  "ncrf2_care.modified_rankin_score.txt"
)
...
dat <- dat %>%
  mutate(across(ncrf2_care.overall_duration_of_invasive_ventilation.txt,
                ~case_when(
                           str_detect(ncrf2_care.overall_duration_of_invasive_ventilation.txt, "6 weeks") ~ "42",
                           str_detect(ncrf2_care.overall_duration_of_invasive_ventilation.txt, "1 month") ~ "30",
                           str_detect(ncrf2_care.overall_duration_of_invasive_ventilation.txt, "\\~ 30 days") ~ "30",
                           str_detect(ncrf2_care.overall_duration_of_invasive_ventilation.txt, "4 weeks") ~ "28",
                           str_detect(ncrf2_care.overall_duration_of_invasive_ventilation.txt, "27\\/03\\/2020 \\& 31\\/03\\/2020 & 17\\/04\\/2020 \\& 12\\/05\\/2020") ~ "4",
                           str_detect(ncrf2_care.overall_duration_of_invasive_ventilation.txt, "approx 7wks") ~ "49",
                           str_detect(ncrf2_care.overall_duration_of_invasive_ventilation.txt, "1-2 days") ~ "2",
                           str_detect(ncrf2_care.overall_duration_of_invasive_ventilation.txt, "\\-") ~ "0",
                           str_detect(ncrf2_care.overall_duration_of_invasive_ventilation.txt, "\\/") ~ "0",
                           str_detect(ncrf2_care.overall_duration_of_invasive_ventilation.txt, "n") ~ "N/A",
                           str_detect(ncrf2_care.overall_duration_of_invasive_ventilation.txt, "N") ~ "N/A",
                           TRUE ~ .)
                )
         )
...
dat <- dat %>%
  mutate(across(ncrf2_care.modified_rankin_score.txt,
                ~case_when(
                           str_detect(ncrf2_care.modified_rankin_score.txt, "n") ~ "N/A",
                           str_detect(ncrf2_care.modified_rankin_score.txt, "N") ~ "N/A",
                           TRUE ~ .)
                )
         )
...
```{r Vector text vars}
variables_text <- c(
  "ncrf2_care.any_other_issue_of_significance_to_note.txt"
)
```
...
```{r Vector date vars}
variables_date <- c(
  "ncrf2_care.icu_or_high_dependency_unit_admission.txt"
)
```
...
dat <- dat %>%
  mutate(across(ncrf2_care.icu_or_high_dependency_unit_admission.txt,
                ~case_when(str_detect(ncrf2_care.icu_or_high_dependency_unit_admission.txt, "27/03/2020 & 31/03/2020 & 17/04/2020 & 12/05/2020				
") ~ "27/03/2020",
                           TRUE ~ .)
                )
         )
...
dat %>%
  saveRDS(
    file = paste0(ilovecovidcns, "/data/latest_freeze/neuro_case_report/ncrf2_care_covidcns_clean.rds")
    )
```