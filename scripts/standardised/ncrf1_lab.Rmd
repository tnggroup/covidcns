---
title: "COVIDCNS NCRF1 Lab Results Cleaning Script"
...
# Read in the data: Neuro Case Report Form Module 1 Lab Results
...
dat %>%
  select(all_of("ncrf1_lab.apttaptr")) %>%
  tbl_summary(
...
dat <- dat %>%
  separate(ncrf1_lab.apttaptr, c("ncrf1_lab.aptt", "ncrf1_lab.aptr"), "/") %>%
  mutate(across(ncrf1_lab.aptr,
                ~case_when(str_detect(ncrf1_lab.aptt, "^0.") ~ ncrf1_lab.aptt,
                           str_detect(ncrf1_lab.aptt, "^1.") ~ ncrf1_lab.aptt,
                           TRUE ~ .)
                )
         ) %>%
  mutate(across(ncrf1_lab.aptt,
                ~case_when(str_detect(ncrf1_lab.aptt, "^0.") ~ NA_character_,
                           str_detect(ncrf1_lab.aptt, "^1.") ~ NA_character_,
                           TRUE ~ .)
                )
         )
...
dat <- dat %>%
  mutate(across(ncrf1_lab.aptt,
                ~case_when(str_detect(ncrf1_lab.aptt, "-36.0") ~ "28.3",
                           TRUE ~ .)
                )
         )
```
...
dat <- dat %>%
  mutate(across(ncrf1_lab.pt,
                ~case_when(str_detect(ncrf1_lab.pt, "H") ~ "13.5",
                           TRUE ~ .)
                )
         )

dat <- dat %>%
  mutate(across(ncrf1_lab.total_bilirubin,
                ~case_when(str_detect(ncrf1_lab.total_bilirubin, "6 ") ~ "6",
                           str_detect(ncrf1_lab.total_bilirubin, "15 ") ~ "15",
                           TRUE ~ .)
                )
         )

dat <- dat %>%
  mutate(across(ncrf1_lab.ddimer,
                ~case_when(str_detect(ncrf1_lab.ddimer, ">18000 ug/L H <500") ~ "18000",
                           TRUE ~ .)
                )
         )
...
lab_results_rr <- read_tsv("lab_results_rr.txt")
