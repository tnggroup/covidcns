...
data_cognitron_raw <- read.table(file = paste0(ilovecovidcns, "/data_raw/cognitron/raw_cognitron/followup2/cognitron_f2_data_19.12.2022.tsv"), sep = '\t', header = FALSE)
...
# Remove _F2 from IDs for compatibility
cognitron_final$user_id <- cognitron_final$user_id %>% str_replace("_F2$", "")
...
saveRDS(object = covid_matching_missing, file = paste0(ilovecovidcns, "/data/cognitron_followup2_matching_missing.rds"))
write_csv(x = covid_matching_missing, file = paste0(ilovecovidcns, "/data_sharing/naomi/cognitron_followup2_matching_missing.csv"))
...
scores_df_wide <- scores_df %>%
  ...
  mutate(
    median_rt = case_when(
      ...
      survey_id == "rs_prospectiveMemoryWords_3_immediate" ~ as.numeric(target_correct_medianRT),
      survey_id == "rs_prospectiveMemoryWords_3_delayed"   ~ as.numeric(target_correct_medianRT),
...
df_new_names <- c(
  "user_id", "os",
  "rs_prospectiveMemoryWords_3_immediate",
  "rs_spatialSpan",
  "rs_manipulations2D",
  "rs_verbalAnalogies",
...
scores_df_final <- scores_df_final %>% dplyr::rename("rs_prospectiveMemoryWords_1_delayed" = "rs_prospectiveMemoryWords_3_delayed")
scores_df_final <- scores_df_final %>% dplyr::rename("rs_prospectiveMemoryWords_1_immediate" = "rs_prospectiveMemoryWords_3_immediate")
scores_df_final <- scores_df_final %>% dplyr::rename("rs_prospectiveMemoryWords_1_delayed_RT" = "rs_prospectiveMemoryWords_3_delayed_RT")
scores_df_final <- scores_df_final %>% dplyr::rename("rs_prospectiveMemoryWords_1_immediate_RT" = "rs_prospectiveMemoryWords_3_immediate_RT")
...
# Add f2 to colnames of copy
colnames(scores_df_copy) <- paste(colnames(scores_df_copy), "f2", sep = "_")
scores_df_copy <- scores_df_copy %>% dplyr::rename("ID" = "ID_f2")

# Remove NAs
scores_df_copy <- na.omit(scores_df_copy)

# Save scores_df_copy to cognitron onedrive
saveRDS(scores_df_copy, paste0(ilovecovidcns, "/data/cognitron/scores/cognitron_scores_and_demographics_f2.rds"))
write.csv(scores_df_copy, paste0(ilovecovidcns, "/data/cognitron/scores/cognitron_scores_and_demographics_f2.csv"))
...
# Add f2 to colnames
colnames(deviation_from_expected_with_users) <- paste(colnames(deviation_from_expected_with_users), "f2", sep = "_")
...
# Add f2 and dfe to colnames
colnames(st_deviation_from_expected_with_users) <- paste(colnames(st_deviation_from_expected_with_users), "f2", sep = "_")
...
ggsave(p_pred_vs_obs_all, filename = paste0(ilovecovidcns, "/data/cognitron/plots/pred_vs_obs_facets_f2.png"),
...
ggsave(p_pred_vs_obs_decade,
       filename = paste0(ilovecovidcns, "/data/cognitron/plots/pred_vs_obs_facets_decade_f2.png"),
       width = 12, height = 12, dpi = 1000)
...
ggsave(p_pred_vs_obs_sex,
       filename = paste0(ilovecovidcns, "/data/cognitron/plots/pred_vs_obs_facets_sex_f2.png"),
       width = 12, height = 12, dpi = 1000)
...
ggsave(p_pred_vs_obs_edu,
       filename = paste0(ilovecovidcns, "/data/cognitron/plots/pred_vs_obs_facets_education_f2.png"),
       width = 12, height = 12, dpi = 1000)
#
## 7.2 Heatmap ----------------------------
...
{
  png(filename = paste0(ilovecovidcns, "/data/cognitron/plots/difference_from_predicted_heatmap_f2.png"),
...
ggsave(p_pred_vs_obs_demo_avg,
       filename = paste0(ilovecovidcns, "/data/cognitron/plots/observed_vs_predicted_barplot_sem_f2.png"),
       width = 12, height = 12, dpi = 1000
)
...
ggsave(p_pred_vs_obs_demo_box,
       filename = paste0(ilovecovidcns, "/data/cognitron/plots/observed_vs_predicted_violin_boxplot_f2.png"),
       width = 12, height = 12, dpi = 1000
)
...
ggsave(p_pred_vs_obs_together,
       filename = paste0(ilovecovidcns, "/data/cognitron/plots/observed_vs_predicted_barplot_together_f2.png"),
...
DfE_plots(deviation_from_expected, plot_fname = paste0(ilovecovidcns, "/data/cognitron/plots/composite_mean_difference_from_expected_f2.png"))
DfE_plots(st_deviation_from_expected, plot_fname = paste0(ilovecovidcns, "/data/cognitron/plots/task_mean_difference_from_expected_f2.png"))
...
ggsave(
  filename = paste0(ilovecovidcns, "/data/cognitron/plots/p_diff_from_exp_facet_age_f2.png"),
...
ggsave(
  filename = paste0(ilovecovidcns, "/data/cognitron/plots/p_diff_from_exp_facet_task_fix_f2.png"),
...
ggsave(
  filename = paste0(ilovecovidcns, "/data/cognitron/plots/p_diff_from_exp_facet_task_free_f2.png"),
...
ggsave(
  filename = paste0(ilovecovidcns, "/data/cognitron/plots/p_diff_from_exp_facet_task_freey_scatter_f2.png"),
  p_diff_from_exp_facet_task_freey_scatter,
  width = 22, height = 6
)
...
ggsave(
  filename = paste0(ilovecovidcns, "/data/cognitron/plots/p_diff_from_exp_facet_group_freey_scatter_f2.png"),
  p_diff_from_exp_facet_group_freey_scatter,
  width = 16, height = 9
)