---
title: "COVIDCNS NIS Inpatient Cleaning Script"
author: "Zain Ahmad"
date: "30/06/2022"
...
# Read in the data: COVIDCNS Neurological Impairment Survey (Inpatient)
...
covidcns_dat <- read_rds(
  file = paste0(ilovecovidcns, "/data_raw/latest_freeze/core_neuro/nis_inp_covid_cns.rds")
  )
...
```{r Check text vars}
dat %>%
  select(all_of("nis_inp.other_observations.txt")) %>%
...
```{r Sumscores inputs}
keys <- seq(1, 1, length.out = 17)

sum_vars <- c(
  "nis_inp.motor.left_upper_limb",
  "nis_inp.motor.right_upper_limb",
  "nis_inp.motor.left_lower_limb",
  "nis_inp.motor.right_lower_limb",
  "nis_inp.motor.trunk",
  "nis_inp.tonejoint_range",
  "nis_inp.sensation",
  "nis_inp.perceptual_function",
  "nis_inp.speech_and_language",
  "nis_inp.cognitive_function",
  "nis_inp.behaviour",
  "nis_inp.mood",
  "nis_inp.seeing_and_vision",
  "nis_inp.hearing",
  "nis_inp.pain",
  "nis_inp.fatigue",
  "nis_inp.other"
...
```{r Add sumscores}
dat <- dat %>% 
  mutate(nis_inp.sum_score = sum_out$scores)
```
...
exclude_cols_numeric <- c(
  "nis_inp.other_observations.txt"
  )
```
...
dat %>%
  saveRDS(
    file = paste0(ilovecovidcns, "/data/latest_freeze/core_neuro/nis_inp_covidcns_clean.rds")
    )
```