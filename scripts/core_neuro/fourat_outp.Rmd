...
attr(dat$fourat_outp.asleep_attempt_patient_address, "labels")
```
...
```{r Recode alertness}
dat <- dat %>% 
  mutate(across(fourat_outp.asleep_attempt_patient_address,
                ~case_when(
                  . == 0 ~ 0,
                  . == 1 ~ 0,
                  . == 2 ~ 4,
...
# Assign new labels
dat$fourat_outp.asleep_attempt_patient_address <- set_labels(dat$fourat_outp.asleep_attempt_patient_address, labels = new_levels_vec)
...
attr(dat$fourat_outp.mental_function_arising_fluctuating, "labels")
```
...
dat <- dat %>% 
  mutate(across(fourat_outp.mental_function_arising_fluctuating,
                ~case_when(
                  . == 1 ~ 4,
                  TRUE ~ .)))
```
...
# Assign new labels
dat$fourat_outp.mental_function_arising_fluctuating <- set_labels(dat$fourat_outp.mental_function_arising_fluctuating, labels = new_levels_vec)
...
```{r Add sumscores}
dat <- dat %>% 
  mutate(fourat_outp.sum_score = sum_out$scores)
```
...
dat <- dat %>%
  mutate(
    fourat_outp.delirium =
      case_when(
        fourat_outp.sum_score >= 4 ~ 2,
        fourat_outp.sum_score < 4 & fourat_outp.sum_score >= 1 ~ 1,
        fourat_outp.sum_score == 0 ~ 0
      )
  )
dat %>%
  select(fourat_outp.delirium) %>%
  tbl_summary()
```
...
# Set question label
dat$fourat_outp.delirium <- sjlabelled::set_label(dat$fourat_outp.delirium, "Is the patient delirious or cognitively impaired?")

# Create vector of answer labels
fourat_outp.delirium_labels <- c(
...
# Set answer labels
dat$fourat_outp.delirium <- sjlabelled::set_labels(dat$fourat_outp.delirium,
                                              labels = fourat_outp.delirium_labels)

# Remove vector of answer labels
rm(fourat_outp.delirium_labels)
