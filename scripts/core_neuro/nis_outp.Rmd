---
title: "COVIDCNS NIS Outpatient Cleaning Script"
author: "Zain Ahmad"
date: "30/06/2022"
...
# Read in the data: COVIDCNS Neurological Impairment Survey (Outpatient)
...
```{r Check text vars}
dat %>%
  select(all_of("nis_outp.other_observations.txt")) %>%
...
sum_vars <- c(
  "nis_outp.motor.left_upper_limb",
  "nis_outp.motor.right_upper_limb",
  "nis_outp.motor.left_lower_limb",
  "nis_outp.motor.right_lower_limb",
  "nis_outp.motor.trunk",
...
```{r Add sumscores}
dat <- dat %>% 
  mutate(nis_outp.sum_score = sum_out$scores)
```
