...
covidcns_dat <- read_csv(
  file = paste0(ilovedata, "/data_raw/covidcns/Covid_CNS_neuro_data_03.03.2022.csv"),
  col_types = list(
...
imp_check(variables = "recruitment_site_name",
          values = vals_cat_4,
          data = dat)
```
...
dat %>% tbl_summary(include = attempted_scan)
```
...
dat %>%
  saveRDS(
    file = paste0(ilovedata, "/data/covidcns/COVID_CNS_neuroimaging_metadata_clean.rds")
    )
```
