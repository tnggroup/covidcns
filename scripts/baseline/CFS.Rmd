...
sum_vars <- c(
  ...
  "cfs.slips_speaking_tongue_month",
...
```{r Add sumscores}
dat <- dat %>% 
  mutate(cfs.sum_score = sum_out$scores)
```
...
Generate physical fatigue subscale scores
...
Add physical fatigue subscale scores as new column
```{r Add physical scores}
dat <- dat %>% 
  mutate(cfs.physical_subscale = sum_out$scores)
```

Generate mental fatigue subscale scores
...
Add mental fatigue subscale scores as new column
```{r Add mental scores}
dat <- dat %>% 
  mutate(cfs.mental_subscale = sum_out$scores)
```
