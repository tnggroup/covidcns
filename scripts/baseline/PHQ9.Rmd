---
title: "COVIDCNS PHQ9 Cleaning Script"
author: "Zain Ahmad"
date: "10/07/2022"
...
# Read in the data: Patient Health Questionnaire 9
...
covidcns_dat <- read_rds(
  file = paste0(ilovecovidcns, "/data_raw/latest_freeze/baseline/phq9_covid_cns.rds")
  )
...
```{r Sumscores inputs}
keys <- seq(1, 1, length.out = 9)

sum_vars <- c(
  "phq9.problems_weeks_bothered.thoughts_that_you_would_be_better_off_dead_or_of_hurting_yourself_in_some_way",
  "phq9.problems_weeks_bothered.feeling_bad_about_yourself_or_felt_that_you_have_let_yourself_or_your_family_down",
  "phq9.problems_weeks_bothered.feeling_down_depressed_or_hopeless",
  "phq9.problems_weeks_bothered.feeling_tired_or_having_little_energy",
  "phq9.problems_weeks_bothered.little_interest_or_pleasure_in_doing_things",
  "phq9.problems_weeks_bothered.moving_or_speaking_so_slowly_that_other_people_could_have_noticed._or_the_opposite_being_so_fidgety_or_restless_that_you_have_been_moving_around_a_lot_more_than_usual",
  "phq9.problems_weeks_bothered.poor_appetite_or_overeating",
  "phq9.problems_weeks_bothered.trouble_falling_or_staying_asleep_or_sleeping_too_much",
  "phq9.problems_weeks_bothered.trouble_concentrating_on_things_such_as_reading_the_newspaper_or_watching_television"
  )
```
...
```{r Add sumscores}
dat <- dat %>% 
  mutate(phq9.sum_score = sum_out$scores)
```
...
dat %>%
  select(phq9.clinical_mdd) %>%
  tbl_summary()
```

Label clinical MDD
```{r Label clinical MDD}
# Set question label
dat$phq9.clinical_mdd <- sjlabelled::set_label(dat$phq9.clinical_mdd, "PHQ9 Clinical MDD")

# Create vector of answer labels
clinical_mdd_labels <- c("No Clinical MDD" = 0, "Clinical MDD" = 1)

# Set answer labels
dat$phq9.clinical_mdd <- sjlabelled::set_labels(dat$phq9.clinical_mdd,
                                                labels = clinical_mdd_labels)

# Remove vector of answer labels
rm(clinical_mdd_labels)
