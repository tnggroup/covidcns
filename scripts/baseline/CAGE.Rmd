...
covidcns_dat <- read_rds(
  file = paste0(ilovedata, "/data_raw/latest_freeze/covid_cns/baseline/cage_covid_cns.rds")
  )
...
Create numeric binary alcoholism investigation variable
...
Create categorical binary alcoholism investigation variable
...
dat <- dat %>%
  mutate(
    cage.investigate =
      recode_factor(
        cage.investigate_numeric,
        "0" = "Do not investigate for alcoholism",
        "1" = "Investigate for alcoholism"
      )
  )
dat %>%
...
dat %>%
  saveRDS(
    file = paste0(ilovedata, "/data/latest_freeze/covidcns/cage_covidcns_clean.rds")
    )
```
