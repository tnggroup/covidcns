---
title: "COVIDCNS Date of COVID Positive"
author: "Zain Ahmad"
date: "01/02/2022"
...
# Read in the data: COVIDCNS Date of COVID Positive
...
covidcns_dat <- read_rds(
  file = paste0(ilovedata, "/data_raw/latest_freeze/covid_cns/baseline/dem_1_covid_cns.rds")
  )
...
exclude_cols_numeric <- c(
  ...
  "startDate",
  "endDate",
  "dem_1.long_ago_diagnosed_required",
  "dem_1.long_ago_diagnosed_required.1"
  )
```
...
covidcns_dat_id <- covidcns_dat %>% #new dataset with ID
  ...
  select(
         ...
         startDate,
         endDate,
         dem_1.diagnosed_required_question_covid19,
         dem_1.long_ago_diagnosed_required,
         dem_1.long_ago_diagnosed_required.1,
         dem_1.diagnosed_covid19_experienced_similar
         ) %>%
...
Inspect variables prior to cleaning: mix of categorical and continuous
```{r COVIDCNS inspect variables}
dat %>%
  tbl_summary(include = c(dem_1.diagnosed_required_question_covid19,
                          dem_1.long_ago_diagnosed_required,
                          dem_1.long_ago_diagnosed_required.1,
                          dem_1.diagnosed_covid19_experienced_similar,
                          dem_1.diagnosed_required_question_covid19_numeric,
                          dem_1.diagnosed_covid19_experienced_similar_numeric),
...
## Long ago diagnosed years
```{r check range of values}
dat %>%
  select("dem_1.long_ago_diagnosed_required") %>%
  tbl_summary()
```

Set limits
Variable name: dem_1.long_ago_diagnosed_required (years)
- Lower: 0, as could be less than a year
- Upper: 2 as of present (Feb 2021), cannot be diagnosed before 2019
*** *Check
```{r define limits}
dem_1.long_ago_diagnosed_required_upper_limit = 2
dem_1.long_ago_diagnosed_required_lower_limit = 0
```

```{r outlier count}
dat %>%
  filter(
    dem_1.long_ago_diagnosed_required > dem_1.long_ago_diagnosed_required_upper_limit | # bigger than the upper limit
      dem_1.long_ago_diagnosed_required < dem_1.long_ago_diagnosed_required_lower_limit & dem_1.long_ago_diagnosed_required > -555 # smaller than the lower limit
        ...
        if_else(
          dem_1.long_ago_diagnosed_required > dem_1.long_ago_diagnosed_required_upper_limit | # bigger than the upper limit
           dem_1.long_ago_diagnosed_required < dem_1.long_ago_diagnosed_required_lower_limit & dem_1.long_ago_diagnosed_required > -555, # smaller than the lower limit
            # or special conditions
          true = -666,
          false = dem_1.long_ago_diagnosed_required,
          missing = NA_real_
        )
    )
```
...
## Long ago diagnosed (months)
```{r check range of values}
dat %>%
  select("dem_1.long_ago_diagnosed_required.1") %>%
  tbl_summary()
```

Set limits
Variable name: dem_1.long_ago_diagnosed_required.1 (months)
- Lower: 0, as could be less than a month
- Upper: 12 (12 months in a year)
*** *Check
```{r define limits}
dem_1.long_ago_diagnosed_required.1_upper_limit = 12
dem_1.long_ago_diagnosed_required.1_lower_limit = 0
```

```{r outlier count}
dat %>%
  filter(
    dem_1.long_ago_diagnosed_required.1 > dem_1.long_ago_diagnosed_required.1_upper_limit | # bigger than the upper limit
      dem_1.long_ago_diagnosed_required.1 < dem_1.long_ago_diagnosed_required.1_lower_limit & dem_1.long_ago_diagnosed_required.1 > -555 # smaller than the lower limit
      # or special conditions
    ) %>%
  nrow()
```
        ...
        if_else(
          dem_1.long_ago_diagnosed_required.1 > dem_1.long_ago_diagnosed_required.1_upper_limit | # bigger than the upper limit
           dem_1.long_ago_diagnosed_required.1 < dem_1.long_ago_diagnosed_required.1_lower_limit & dem_1.long_ago_diagnosed_required.1 > -555, # smaller than the lower limit
            # or special conditions
          true = -666,
          false = dem_1.long_ago_diagnosed_required.1,
          missing = NA_real_
        )
    )
```
...
dat %>%
  saveRDS(
    file = paste0(ilovedata, "/data/latest_freeze/covidcns/covidpositive_covidcns_clean.rds")
    )
```
