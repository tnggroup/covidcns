...
```{r COVIDCNS inspect variables initial}
dat %>%
...
```{r Recode outliers to -666 years}
dat <- dat %>%
    mutate(
      dem.long_ago_diagnosed_required = 
        if_else(
...
```{r inspect after recoding to -666 years}
dat %>%
  select("dem.long_ago_diagnosed_required") %>%
  tbl_summary()
```
...
```{r check range of values months}
dat %>%
...
```{r Recode outliers to -666 months}
dat <- dat %>%
    mutate(
      dem.long_ago_diagnosed_required.1 = 
        if_else(
...
```{r inspect after recoding to -666 months}
dat %>%
  select("dem.long_ago_diagnosed_required.1") %>%
  tbl_summary()
```
