---
title: "COVIDCNS PCL5 Cleaning Script"
author: "Zain Ahmad"
date: "10/07/2022"
...
# Read in the data: Posttraumatic Stress Disorder Checklist for DSM-5
...
```{r Sumscores inputs}
keys <- seq(1, 1, length.out = 20)

sum_vars <- c(
  "pcl5.past_month_bothered.repeated_disturbing_and_unwanted_memories_of_the_stressful_experience",
  "pcl5.past_month_bothered.repeated_disturbing_dreams_of_the_stressful_experience",
  "pcl5.past_month_bothered.suddenly_feeling_or_acting_as_if_the_stressful_experience_were_actually_happening_again",
  "pcl5.past_month_bothered.feeling_very_upset_when_something_reminded_you_of_the_stressful_experience",
  "pcl5.past_month_bothered.having_strong_physical_reactions_when_something_reminded_you_of_the_stressful_experience_",
  "pcl5.past_month_bothered.avoiding_memories_thoughts_or_feelings_related_to_the_stressful_experience",
  "pcl5.past_month_bothered.avoiding_external_reminders_of_the_stressful_experience_",
  "pcl5.past_month_bothered.trouble_remembering_important_parts_of_the_stressful_experience",
  "pcl5.past_month_bothered.having_strong_negative_beliefs_about_yourself_other_people_or_the_world_",
  "pcl5.past_month_bothered.blaming_yourself_or_someone_else_for_the_stressful_experience_or_what_happened_after_it",
  "pcl5.past_month_bothered.having_strong_negative_feelings_such_as_fear_horror_anger_guilt_or_shame",
  "pcl5.past_month_bothered.loss_of_interest_in_the_activities_that_you_used_to_enjoy",
  "pcl5.past_month_bothered.feeling_distant_or_cut_off_from_from_other_people",
  "pcl5.past_month_bothered.trouble_experiencing_positive_feelings_",
  "pcl5.past_month_bothered.irritable_behaviour_angry_outburst_or_acting_aggressively",
  "pcl5.past_month_bothered.taking_too_many_risks_or_doing_things_that_could_cause_you_harm",
  "pcl5.past_month_bothered.being_superalert_or_watchful_or_on_guard",
  "pcl5.past_month_bothered.feeling_jumpy_or_easily_startled",
  "pcl5.past_month_bothered.having_difficulty_concentrating",
  "pcl5.past_month_bothered.trouble_falling_or_staying_sleep"
  )
```
...
```{r Add sumscores}
dat <- dat %>% 
  mutate(pcl5.sum_score = sum_out$scores)
```
...
Create clinical PTSD variable
```{r Create clinical PTSD}
dat <- dat %>%
  mutate(
    pcl5.clinical_ptsd =
      case_when(
...
dat %>%
  select(pcl5.clinical_ptsd) %>%
  tbl_summary()
```

Label clinical PTSD
```{r Label clinical PTSD}
# Set question label
dat$pcl5.clinical_ptsd <- sjlabelled::set_label(dat$pcl5.clinical_ptsd, "Clinical PTSD")

# Create vector of answer labels
clinical_ptsd_labels <- c("No Clinical PTSD" = 0, "Clinical PTSD" = 1)

# Set answer labels
dat$pcl5.clinical_ptsd <- sjlabelled::set_labels(dat$pcl5.clinical_ptsd,
                                                 labels = clinical_ptsd_labels)

# Remove vector of answer labels
rm(clinical_ptsd_labels)
