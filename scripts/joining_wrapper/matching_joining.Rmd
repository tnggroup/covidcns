...
# Initial empty vector of rows to change IDs for in matching_joined
swapping_rows <- c()

# Create vector of rows corresponding to IDs that need changing
for (i in swapped_ids$incorrect_ID) {
  swapping_rows <- append(x = swapping_rows, values = which(matching_joined$ID == i))
}

# Add row numbers to swapped_ids data frame
swapped_ids <- cbind(swapped_ids, swapping_rows)

# Swap IDs in matching_joined by row number
for (i in 1:length(swapped_ids)) {
  matching_joined[swapped_ids[i,3], 1] <- swapped_ids[i,1]
}

```

Coalesce age sex demographic variables from baseline and ncrf dem
...
data_export_matching <- matching_joined %>%
  select(
      "ID",
