...
### mh_case_report_covidcns_clean
Read in & variable names
```{r mh_case_report_covidcns_clean Read in data & variable names}
mh_case_report_covidcns_clean <-
  read_rds(
    file = 
      paste0(ilovecovidcns, "/data/latest_freeze/mh_case_report/mh_case_report_covidcns_clean.rds")
  )
...
Dimensions
```{r mh_case_report_covidcns_clean Dimensions}
mh_case_report_covidcns_clean %>%
  colnames()
```

Select columns for joining
```{r mh_case_report_covidcns_clean Select columns for joining}
mh_case_report_covidcns_clean_selected <-
  mh_case_report_covidcns_clean %>%
  select(
    -all_of(exclude_variables)
  )
...
## List of tibbles to join, neuro data only
```{r List of tibbles to join}
neuro_tibble_list <- 
  list(
    # clin_neuro
...
# Remove ids with issues
neuro_data_joined <- neuro_data_joined %>%
  filter(!ID %in% swapped_ids$incorrect_ID)
...
data_joined <- 
  plyr::join_all(
    dfs =  list(data_joined, case_def_dat),
    by = c("ID"),
    type = "full"
    )
```
...
# Save neuro data set
```{r matching Save neuro data set sharing}
saveRDS(
  object = neuro_data_joined, 
  file = paste0(ilovecovidcns, "/data_sharing/glynn/covidcns_neuro_data.rds")
)
```
