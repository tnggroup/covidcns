---
title: "COVIDCNS Clinical NA (Original) Cleaning Script"
author: "Zain Ahmad"
date: "19/10/2022"
...
# Read in the data: Clinical Neurological Assessment (Original)
...
covidcns_dat <- read_rds(
  file = paste0(ilovecovidcns, "/data_raw/comb_neuro_assess/clin_neuro/clinical_na_ori_covidcns.rds")
  )
...
```{r Vector text vars}
variables_text <- c(
  "clinical_na_ori.dysarthria.1",
  "clinical_na_ori.nystagmus.1",
  "clinical_na_ori.if_abnormal_please_describe.txt",
  "clinical_na_ori.other_observations.txt"
)
```
...
Import patient status data
```{r Import status}
status_dat <- read_rds(
  file = paste0(ilovecovidcns, "/data_raw/comb_neuro_assess/core_neuro/core_inp_outp_covidcns.rds")
  )
...
# Change colname to "ID" and remove duplicate IDs
...
Join status data to main data frame
```{r Join status data}
dat <- plyr::join_all(
  dfs = list(dat,
             status_dat),
...
Split into inpatient and outpatient
```{r}
...
dat %>%
  saveRDS(
    file = paste0(ilovecovidcns, "/data/latest_freeze/clin_neuro/clinical_na_ori_covidcns_clean.rds")
    )
```